 ...
 * 
 * <p>
 * This test suite measures and compares the performance of all optimized FFT
 * implementations against the base implementation, providing detailed speedup
 * metrics and performance characteristics analysis.
 * </p>
 */
...
class FFTPerformanceBenchmarkTest {
    ...
    class ComprehensiveAnalysis {
            ...
            for (int size : SIZES) {
                try {
                    ...
                    if (optimized != null && !(optimized instanceof FFTBase)) {
                        ...
                        String status = result.speedup > 1.2 ? "✓ GOOD" : result.speedup > 1.0 ? "~ OK" : "✗ SLOW";
        ...
        void shouldVerifyCorrectnessOfAllOptimizedImplementations() {
            ...
            for (int size : SIZES) {
                if (size > 8192)
                    continue; // Skip very large sizes for correctness tests
            ...
            // Most correctness tests should pass (allowing for some implementations with
            // known issues)
    ...
    /**
     * Verifies correctness of an optimized implementation against the base
     * implementation.
     */
